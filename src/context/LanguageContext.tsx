...
const STORAGE_KEY = '@enjazaty/language';
...
interface LanguageContextValue {
  ...
  /** True until the persisted language has been loaded. */
  loading: boolean;
}
...
/** Applies text direction on web (and via I18nManager on native). */
function applyDirection(isRTL: boolean) {
  if (Platform.OS === 'web' && typeof document !== 'undefined') {
    document.documentElement.dir = isRTL ? 'rtl' : 'ltr';
    document.documentElement.lang = isRTL ? 'ar' : 'en';
  } else {
    // On native, force direction. A full restart is needed for native to
    // re-layout, but we set it so subsequent screens render correctly.
    try {
...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  // Arabic is the default (RTL-first product).
